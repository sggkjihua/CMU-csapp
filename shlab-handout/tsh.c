...
// 这个应该是解析的指令，如果输入的指令是一个内嵌的指令，那么我们就直接执行
// 否则就需要去fork一个子进程，然后再去运行相应的指令
void eval(char *cmdline) 
{
    char *argv[MAXARGS];
    char buf[MAXLINE];
 ...
 */
// 内核发送SIGCHLD 到所有停止了的子进程，或者收到了 SIGSTOP 或者 SIGTSTP的信号的
// handler 将要杀死所有的僵尸进程，但是不需要去等待其它未完成的子进程
void sigchld_handler(int sig) 
{
